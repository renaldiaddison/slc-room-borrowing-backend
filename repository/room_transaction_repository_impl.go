...
func (repository *RoomTransactionRepositoryImpl) CreateRoomTransactionReturn(ctx context.Context, tx *sql.Tx, roomTransaction entities.RoomTransaction) entities.RoomTransaction {
	...
	_, err := tx.ExecContext(ctx, SQL, roomTransaction.ReturnerUsername, roomTransaction.ReturnerDivision, roomTransaction.RoomOut.Format("2006-01-02 15:04:05"))
	helper.PanicIfError(err)
	return roomTransaction
}

func (repository *RoomTransactionRepositoryImpl) FindActiveRoomTransaction(ctx context.Context, tx *sql.Tx) []entities.RoomTransaction {
	SQL := "SELECT * FROM roomtransactions WHERE room_out IS NULL"
...
//func (repository *RoomTransactionRepositoryImpl) FindNotActiveRoomTransaction(ctx context.Context, tx *sql.Tx) []entities.RoomTransaction {
//	SQL := "SELECT * FROM roomtransactions WHERE room_out IS NOT NULL"
//	rows, err := tx.QueryContext(ctx, SQL)
//	helper.PanicIfError(err)
//	defer rows.Close()
//
//	var notActiveRoomTransactions []entities.RoomTransaction
//}

func (repository *RoomTransactionRepositoryImpl) FindAllRoomTransaction(ctx context.Context, tx *sql.Tx) []entities.RoomTransaction {
	SQL := "SELECT * FROM roomtransactions"
	rows, err := tx.QueryContext(ctx, SQL)
	helper.PanicIfError(err)
	defer func(rows *sql.Rows) {
		err := rows.Close()
